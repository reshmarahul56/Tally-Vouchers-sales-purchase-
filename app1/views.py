...
def add_purchase(request):
    if 't_id' in request.session:
        ...
        if request.method=='POST':
            ...
            pur_no=request.POST['no']
            invoice=request.POST['invoiceno']
            date=request.POST['date']
            ...
            purch.save()
            print("added")
            return redirect('vouchers_purchase')
        return render(request,'Vouchers_purchase.html',{'tally':tally})
    return redirect('/')
    
def vouchers_sales(request):
     if 't_id' in request.session:
        if request.session.has_key('t_id'):
            t_id = request.session['t_id']
        else:
